...
template <typename T> NB_INLINE void bind_base(ArrayBinding &b) {
    ...
    b.set_item = [](PyObject *o, Py_ssize_t i_, PyObject *value) noexcept -> int {
        ...
        if (i < size) {
            ...
            if (success) {
                using Casted = nb::detail::intrinsic_t<Value>;
                using Out = std::conditional_t<std::is_pointer_v<Value>, Casted*, Casted &>;

                Out casted = in.operator Out();
                inst->set_entry(i, (value_t<T>) casted);
            }
            cleanup.release();
